...
const AccordionContent = React.forwardRef<React.ElementRef<typeof AccordionPrimitive.Content>, React.ComponentPropsWithoutRef<typeof AccordionPrimitive.Content>>(({ className, children, ...props }, ref) => <><style>{`@keyframes loam-accordion-down{from{height:0;opacity:0}to{height:var(--radix-accordion-content-height);opacity:1}}@keyframes loam-accordion-up{from{height:var(--radix-accordion-content-height);opacity:1}to{height:0;opacity:0}}@media(prefers-reduced-motion:reduce){.loam-accordion-motion{animation-duration:.01ms!important}}`}</style><AccordionPrimitive.Content ref={ref} className="loam-accordion-motion overflow-hidden text-sm text-[var(--loam-ink-soft)] data-[state=open]:[animation:loam-accordion-down_420ms_var(--loam-ease-spring)] data-[state=closed]:[animation:loam-accordion-up_280ms_var(--loam-ease-spring)]" {...props}><div className={clsx("pb-5 pr-12 leading-relaxed", className)}>{children}</div></AccordionPrimitive.Content></>)
