...
const AccordionTrigger = React.forwardRef<React.ElementRef<typeof AccordionPrimitive.Trigger>, React.ComponentPropsWithoutRef<typeof AccordionPrimitive.Trigger>>(({ className, children, ...props }, ref) => <AccordionPrimitive.Header className="flex"><AccordionPrimitive.Trigger ref={ref} className={clsx("group flex min-h-16 flex-1 items-center justify-between py-4 text-left font-serif text-xl font-medium outline-none transition-colors hover:text-[var(--loam-clay)] focus-visible:ring-3 focus-visible:ring-[var(--loam-focus)]", className)} {...props}>{children}<span className="ml-4 grid size-8 shrink-0 place-items-center rounded-full border border-[var(--loam-ink)] bg-[var(--loam-surface)] shadow-[0_2px_0_var(--loam-ink)]"><ChevronDown className="size-4 transition-transform duration-300 ease-[var(--loam-ease-spring)] group-data-[state=open]:rotate-180 motion-reduce:transition-none"/></span></AccordionPrimitive.Trigger></AccordionPrimitive.Header>)
